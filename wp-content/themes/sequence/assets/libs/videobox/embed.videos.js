  ...
  $.fn.embedVideo = function(options) {
    $(this).each(function() {
      var settings = $.extend({
            ...
            play : ($(this).data('icon')) ? $(this).data('icon') : '<img src="http://360sequence.com/wp-content/uploads/2018/05/play_youtube.png" >'
          }, options);
      ...
      var video_styles = function() {
        video_cont.css({
          ...
          // 'background-image': 'url("http://360sequence.com/wp-content/uploads/2018/05/screenshot-1.png")'
          'background-image': 'url("http://360sequence.com/wp-content/uploads/2018/07/screenshot.jpg")'
        });
      }
      ...
      var videoContainer = $('<div>').attr({
        'style' : 'width: 100%; margin: 0 auto; max-width: 100%;' + settings.maxWidth + ';',
        'class' : 'videoContainer'
      });
      var frameVideoContainer = $('<div>').attr({
        'style' : 'position: relative; height: 100%; width: 89%; margin:0 auto;  padding-bottom: 50%;',
      ...
      var playButton = $('<a>').attr({
        'style' : 'position: absolute; cursor: pointer; width: 73px; height: 55px; top: 50%; left: 50%;  transform: translate(-50%,-50%); text-decoration: none; border: none; border-radius: 50%; box-shadow: 0 0 100px rgba(0, 0, 0, 0.7); opacity: 0.7;'
      }).append(playIcon);
